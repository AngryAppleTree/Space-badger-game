...
class GameScene extends Phaser.Scene {
  ...
  preload() {
    this.load.image('player', '/spacebadgergame/space-badger.png');
    this.load.image('alien', '/spacebadgergame/polecat.png');
    this.load.image('bullet', '/spacebadgergame/star.png');

    // Load all playlist tracks
    this.playlist.forEach((track, index) => {
      this.load.audio(`track${index}`, `/spacebadgergame/${track}`);
    });
  }
